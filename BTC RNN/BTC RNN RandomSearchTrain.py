...
# === Build RNN model ===
def build_rnn(input_shape, units1, units2, dropout):
    model = Sequential()
    model.add(Input(shape=input_shape))
    model.add(SimpleRNN(units1, return_sequences=True, activation="tanh"))
    model.add(Dropout(dropout))
    model.add(SimpleRNN(units2, return_sequences=False, activation="tanh"))
    model.add(Dropout(dropout))
    model.add(Dense(1))
...
# === Load training data ===
train_path = 'C:/Users/User/Desktop/RNNLSTM/BTC RNN/data/BTC_USDT_Train_2018-01-01_to_2019-12-31_4h.csv'
...
# Save scaler
joblib.dump(sc, 'C:/Users/User/Desktop/RNNLSTM/BTC RNN/data/Train_data/minmax_scaler.pkl')
...
for _ in range(20):
    param_grid.append({
        'units1': random.choice([32, 50, 64]),
        'units2': random.choice([16, 32, 50]),
...
for i, params in enumerate(param_grid):
    print(f"\n🔁 Training {i+1}/20: {params}")
    start_time = time.time()

    model = build_rnn((look_back, 1), params['units1'], params['units2'], params['dropout'])
    ...
    print(f"⏱ {elapsed:.2f}s | RMSE: {rmse:.4f}")
    params['rmse'] = rmse
...
# === Save best model ===
model_path = f"C:/Users/User/Desktop/RNNLSTM/BTC RNN/data/Train_data/bitcoin_RNN_model{look_back}.keras"
